...
import { selectMusicSuggestion } from './lib.mjs';
...
const musicSuggestion = post.musicSuggestion || selectMusicSuggestion(post.title, post.category);
...
console.log(`Passende Instagram-Musik: "${musicSuggestion.title}" von ${musicSuggestion.artist}. Die offizielle API kann Musik nicht an Karussells anhängen; in Instagram über „Musik hinzufügen“ nach „${musicSuggestion.searchQuery}“ suchen.`);
await fs.writeFile('published.json', JSON.stringify({ ...post, musicSuggestion, instagramMediaId: published.id, imageUrls, publishedAt: new Date().toISOString() }, null, 2));
